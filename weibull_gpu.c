...
#include <assert.h>
  ...
  int weibull_fit_gpu_verbose_debug=0;
#define WEIBULL_ERROR_HANDLER(x,msg)
#else
  int weibull_fit_gpu_verbose_debug=1;
  static int  tthrow(int x, const char* msg){if(weibull_fit_gpu_verbose_debug) fprintf(stderr,"%s\n",msg); return x;}
#define WEIBULL_ERROR_HANDLER(x,msg) return tthrow(x,msg)
#endif
  
  
  
  double weibull_pdf_gpu(double x, double scale, double shape)
  {
    double pdf;
   ...
   *
   */
  double weibull_cdf_gpu(double x, double scale, double shape)
  {
    double cdf;
    double tempVal, tempVal1;
   ...
   *
   */
  double weibull_inv_gpu(double x, double scale, double shape)
  {
    double inv;
  ...
  void printWeibullBuildInfo_gpu(FILE *fh)
  {
    if(fh == NULL)
      fh = stdout;
#ifdef HAVE_CONFIG_H
