...
int MetaRecognition::EvtGeneric(double* inputData, int inputDataSize, int inward, double x)
{
  ...
  int icnt=0;
  if(!inward && (sign > 0) ) {
    printf("In the first if condition ...\n");
    icnt = inputDataSize;
  ...
  printf("In EvtGeneric function, printing weibull parameters -- before\n");
  printf("    fitting_size = %d \n", fitting_size);
  printf("    alpha = %f \n", alpha);
  printf("    *(dataPtr) = %f \n", *(dataPtr));
  printf("    *(parmci) = %f \n", *(parmci));
  ...
  printf("In EvtGeneric function, printing weibull parameters -- after\n");
  printf("    fitting_size = %d \n", fitting_size);
  printf("    alpha = %f \n", alpha);
  printf("    *(dataPtr) = %f \n", *(dataPtr));
  printf("    *(parmci) = %f \n", *(parmci));
...
{
  printf("In FitHigh function ...\n");
  if(fsize>0)    fitting_size=fsize;
  sign = 1;
  return EvtGeneric(inputData, inputDataSize);
}

int MetaRecognition::FitSVM(svm_node_libsvm* SVMdata, int inputDataSize, int label_of_interest, bool label_has_positive_score, MR_fitting_type fit_type, int fit_size )
{
  Reset();
  ftype=fit_type;
